...
const config: Config = {
  ...
  roomInactiveTimeout: parseInt(
    process.env.ROOM_INACTIVE_TIMEOUT || "3600000",
    10,
  ),
...
if (
  Number.isNaN(config.roomCleanupInterval) ||
  config.roomCleanupInterval <= 0
) {
  throw new Error(
...
if (
  Number.isNaN(config.roomInactiveTimeout) ||
  config.roomInactiveTimeout <= 0
) {
  throw new Error(
...
if (config.limits.maxPayload <= 0 || Number.isNaN(config.limits.maxPayload)) {
  throw new Error(
    `Invalid MAX_PAYLOAD: ${process.env.MAX_PAYLOAD}. Must be a positive number.`,
  );
}

if (
  config.limits.maxConnsPerIp <= 0 ||
  Number.isNaN(config.limits.maxConnsPerIp)
) {
  throw new Error(
...
if (
  config.limits.maxGlobalConns <= 0 ||
  Number.isNaN(config.limits.maxGlobalConns)
) {
  throw new Error(
...
if (
  config.limits.maxConnRatePerMin <= 0 ||
  Number.isNaN(config.limits.maxConnRatePerMin)
) {
  throw new Error(
...
if (
  config.limits.maxMsgRatePerSec <= 0 ||
  Number.isNaN(config.limits.maxMsgRatePerSec)
) {
  throw new Error(
