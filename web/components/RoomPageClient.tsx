...
export default function RoomPageClient({ roomId }: RoomPageClientProps) {
  ...
  useEffect(() => {
    const doc = new Y.Doc();
    const wsProvider = new WebsocketProvider(config.realtime.wsUrl, roomId, doc);
  ...
  const handleJoin = (info: { name: string; color: string }) => {
    setUserInfo(info);
    setHasJoined(true);
  };
  ...
  return (
    <div className="flex h-screen w-full flex-col overflow-hidden bg-zinc-950 text-white">
      {!hasJoined && <JoinRoomModal onJoin={handleJoin} provider={provider} />}
      ...
      <footer className="h-9 border-t border-zinc-800 bg-zinc-900/80 backdrop-blur flex items-center justify-between px-4 z-10">
        <div className="flex items-center gap-2 group cursor-pointer">
          <div className={`h-2 w-2 rounded-full ${hasJoined ? 'bg-emerald-500' : 'bg-yellow-500'}`}></div>
